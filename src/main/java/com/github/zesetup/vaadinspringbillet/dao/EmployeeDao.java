...
public class EmployeeDao {
  ...
  public List<Employee> load(String sortField, Boolean isAsc, Integer recordsOffset,
    ...
    if (fullSearch != null) {
      criteriaQuery
          .where(fullSearchToPredicates(fullSearch, criteriaBuilder, criteriaQuery, employeeRoot));
    }
    if (isAsc == null)
      isAsc = true;
    if (sortField == null) {
    ...
    logger.info("** DEV result size:" + result.size() + " offset=" + recordsOffset + " limit="
        + recordsLimit + " sort=" + sortField);
    return result;
  }

  private Predicate[] fullSearchToPredicates(String fullSearch, CriteriaBuilder criteriaBuilder,
      CriteriaQuery<?> criteriaQuery, Root<Employee> employeeRoot) {
    List<Predicate> predicateList = new ArrayList<Predicate>();
    Predicate predicate =
        criteriaBuilder.like(criteriaBuilder.upper(employeeRoot.<String>get("name")),
            "%" + fullSearch.toUpperCase() + "%");
    Predicate predicate2 =
        criteriaBuilder.like(criteriaBuilder.upper(employeeRoot.<String>get("surname")),
            "%" + fullSearch.toUpperCase() + "%");
